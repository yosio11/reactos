...
#define _SEH3$_NESTED_FUNC_OPEN(_Name) \
    ...
    { \
        ...
        if (Action == 0) { _SEH3$FILTER_RESULT result = {(int)&_SEH3$_TrylevelFrame}; return result; }
...
\
        ...
        { _SEH3$FILTER_RESULT result = {(expression)}; return result; } \
    }
...
\
        /* This construct makes sure that the finally function returns */ \
        /* a proper value at the end */ \
        for (; ; (void)({ _SEH3$FILTER_RESULT result = {(0)}; return result; 0;}))
